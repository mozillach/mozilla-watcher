...
class GitHubHelper {
  ...
  async getAll() {
    ...
    for (const orgName of this.orgNames) {
      try {
        await this.getNewRepos(orgName);
        debug(`Completed fetching repos for ${orgName}`);
      } catch (error) {
  ...
  fetchPagesRecursively(page, orgName, resolve, reject) {
    ...
    fetch(fullUrl, options).then((res) => res.json())
      .then((repositories) => {
        ...
        if (invalidResponse) {
          debug(`Invalid response for ${orgName} on page ${page}:`, repositories);
