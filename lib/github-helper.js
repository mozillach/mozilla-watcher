...
class GitHubHelper {
  ...
  getNewRepos(lastCheckDate) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.fetchPagesRecursively(1, this.repos, resolve, reject);
    });
  }

  fetchPagesRecursively(page, repos, resolve, reject) {
    ...
    fetch(fullUrl, options).then((res) => {
      ...
      if (repositories && repositories.length === 100) {
        ...
        this.fetchPagesRecursively(++page, repos, resolve, reject);
      } else {
        resolve(repos);
      }
    }).catch((err) => {
