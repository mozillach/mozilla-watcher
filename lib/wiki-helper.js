...
class WikiHelper {
  constructor() {
    this.edits = [];
    this.apiUrl = 'https://wiki.mozilla.org/api.php?action=query&list=recentchanges&rcprop=title&rclimit=500&format=json';
    this.lastRun = null;
  }
  ...
  groupEdits(edits) {
    ...
    edits.forEach((change) => {
      if (!groupedEdits.includes(change.title)) {
        groupedEdits.push(change.title);
      }
    });
