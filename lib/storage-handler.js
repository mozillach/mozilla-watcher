 ...
 */
class StorageHandler extends EventEmitter {
  constructor() {
    super();
  ...
  async saveDifference(lastCheck, newRepositories) {
    ...
    if(allRepositories.length < MAX_RETURN_ROWS_NEW) {
      ...
      let existingRepositories = this.getRedisAsync('REPOSITORIES') || [];
  ...
  async getLastRepoCheckDate() {
    ...
    const lastCheckDate = this.getRedisAsync('LAST_REPO_UPDATE');
